...
class MemoryCompressor:
    ...
    def compress_if_needed(state: AgentState, memory_config=None, llm_client=None) -> bool:
        ...
        if len(state.memory.important_decisions) > important_decisions_keep:
            state.memory.important_decisions = state.memory.important_decisions[-important_decisions_keep:]
            logger.info(f"[MemoryCompressor] 修剪 important_decisions 到 {important_decisions_keep} 个")
            compressed = True
        ...
        if memory_config and memory_config.llm_compression_enabled and llm_client:
            try:
                ...
                if memory_size > memory_config.llm_compression_threshold:
                    logger.info(f"[MemoryCompressor] Memory size ({memory_size} chars) exceeds LLM compression threshold ({memory_config.llm_compression_threshold}), triggering LLM compression")
                    MemoryCompressor._compress_with_llm(state, memory_config, llm_client)
                    compressed = True
            except Exception as e:
                logger.warning(f"[MemoryCompressor] LLM compression failed: {e}, continuing with rule-based compression only")
        ...
        if memory_config and memory_config.deduplication_enabled:
            try:
                ...
                logger.warning(f"[MemoryCompressor] Deduplication failed: {e}, continuing without deduplication")
    ...
    def _compress_attempts(state: AgentState, keep_recent: int = None) -> None:
        ...
        logger.info(f"[MemoryCompressor] 压缩了 {len(old)} 个旧 attempts，保留 {len(recent)} 个最近的")
    ...
    def _compress_decisions(state: AgentState, keep_recent: int = None) -> None:
        ...
        logger.info(f"[MemoryCompressor] 压缩了 {len(old)} 个旧 decisions，保留 {len(recent)} 个最近的")
...
- 重要里程碑：..."""
            ...
            logger.info(f"[MemoryCompressor] ✅ LLM 压缩完成: {len(old_decisions)} 个决策压缩为摘要，保留 {len(recent_decisions)} 个最近的")
    ...
    def _deduplicate_memory(state: AgentState, memory_config) -> bool:
        ...
        if len(state.memory.decisions) > 1:
            ...
            if deduplicated:
                ...
                logger.info(f"[MemoryCompressor] 去重完成: {original_count} 个决策 -> {len(unique_decisions)} 个唯一决策")
        ...
        if len(state.memory.attempts) > 1:
                ...
                if signature not in seen_attempt_signatures:
                    ...
                    for existing in unique_attempts:
                        if existing.get("type") in ["compressed", "llm_compressed"]:
                            continue
                        similarity = MemoryCompressor._calculate_attempt_similarity(attempt, existing)
                        if similarity >= memory_config.deduplication_similarity_threshold:
                            similar = True
                            break
            ...
            if deduplicated:
                ...
                logger.info(f"[MemoryCompressor] 去重完成: {original_count} 个尝试 -> {len(unique_attempts)} 个唯一尝试")
