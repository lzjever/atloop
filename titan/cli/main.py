"""TITAN CLI - minimal implementation (uses varlord for CLI argument parsing)."""
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="TITAN - Task Automation Node")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # init
    init_parser = subparsers.add_parser("init", help="Initialize configuration")
    add_titan_dir_arg(init_parser)

    # execute (only execution method)
    execute_parser = subparsers.add_parser("execute", help="Execute a task")
    add_titan_dir_arg(execute_parser)
    ...
    add_titan_dir_arg(config_parser)

    return parser


def add_titan_dir_arg(parser: argparse.ArgumentParser) -> None:
    """Add titan-dir argument."""
    parser.add_argument("--titan-dir", help="Custom config directory")
