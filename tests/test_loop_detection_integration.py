...
from atloop.memory.state import AgentState, Memory, BudgetUsed
...
def mock_coordinator_with_loop_detection():
    ...
    coordinator.loop_detector = LoopDetector(LoopDetectionConfig(
...
class TestActPhaseLoopDetectionIntegration:
    ...
    def test_act_phase_records_actions_to_progress_tracker(self, mock_coordinator_with_loop_detection):
    ...
    def test_act_phase_preserves_action_history_in_memory(self, mock_coordinator_with_loop_detection):
...
class TestInterventionMessageContent:
    ...
    def test_intervention_includes_evidence(self):
        ...
        if analysis.is_looping:
            ...
            prompt = intervention.prompt_injection
            assert "evidence" in prompt.lower() or "pattern" in prompt.lower() or \
                   "repeat" in prompt.lower()
