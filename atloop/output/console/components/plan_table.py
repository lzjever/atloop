...
from atloop.output.events import OutputEvent, LLMResultEvent
...
class PlanTableComponent(FormattingComponent):
    ...
    def format(
        ...
        if plan_info.get("plan_items"):
            ...
            plan_text = "\n".join(
                f"  {i+1}. {item}" for i, item in enumerate(plan_items[:10])
            )
        ...
        if plan_info.get("planned_actions"):
            actions_text = "\n".join(
                f"  • {action}" for action in plan_info["planned_actions"]
            )
                ...
                if "plan" in data and data["plan"]:
                    ...
                    if isinstance(plan_data, list):
                        plan_info["plan_items"] = [
                            str(item) for item in plan_data
                        ]
