    ...
    def get_detailed_description(self) -> str:
        ...
        for line in lines:
            ...
            if line_stripped.startswith("**"):
                ...
                elif "⚠️" in section_name and (
                    "Important" in section_name or "WARNING" in section_name
                ):
        ...
        try:
            ...
            if execute_doc:
                ...
                if examples_match:
                    ...
                    if examples_text:
                        parts.append(f"\n**Examples**:\n{examples_text}")
        except (AttributeError, IndexError, re.error, TypeError):
            # If extraction fails (missing attribute, index error, regex error, type error), continue without parameters/examples
            pass
        ...
        try:
            desc_str = (
                self.description if isinstance(self.description, str) else str(self.description)
            )
            if len(result) < len(desc_str) + 50:
                result = f"{desc_str}\n\n{result}"
        except (AttributeError, TypeError):
            # If description access fails (missing attribute, type error), just return what we have
            pass
